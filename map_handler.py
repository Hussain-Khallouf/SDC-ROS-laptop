...
class MapHandler(metaclass=SingletonMeta):
    def __init__(self, map_nodes: List[MapNode], start_node_name: str = "start"):
        self.map = map_nodes
        self.node_counts = len(map_nodes)
        self.current_node = next(
            (node for node in self.map if node.name == start_node_name)
        )
    ...
    def get_node_by_name(self, name):
        return next((node for node in self.map if node.name == name))
    ...
    def __str__(self):
        for node in self.map:
            print(f"{node.name}: next nodes = {node.next_nodes}")
