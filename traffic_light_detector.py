...
vertices = np.array([ [int(width * 0.5), int(height * 0.25)],[int(width * 0.75) ,int(height * 0.25)] , [width , int(height * 0.4)],[width,height],[int(width * 0.5),height] ])

def traffic_viewer(msg: CompressedImage):
    global vertices
    img_Original = decode_image(msg.data)
    
    interestedImage = region_of_interest_Black_Background(img_Original,vertices=vertices, is_white=True)
    ...
    if big is not None:
            ...
            if(contourArea > 6000):
                traffic_Light = region_of_interest_Black_Background(img_With_Out_Line,vertices=big)
                color_Selected = get_Color(traffic_Light)
                if color_Selected == "Move":
                    img_Original , sign , (x,y) = boundaryBox(interestedImage , big ,0,"Move")
                    cv2.putText(img=img_Original, text=color_Selected, org=(x+10, y-5), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.7, color=(0, 255, 0),thickness=1)
                else:
                    img_Original , sign , (x,y) = boundaryBox(interestedImage , big ,0,"Stop")
                    cv2.putText(img=img_Original, text=color_Selected, org=(x+10, y-5), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.7, color=(0, 0, 255),thickness=1)
                cv2.imshow('sign', traffic_Light)
            
    cv2.imshow('interestedImage', interestedImage)
    cv2.waitKey(1)
