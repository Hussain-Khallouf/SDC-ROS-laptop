...
model = load_model("/home/hussain/project2_ws/src/self_driving_car_01/scripts/model-3x3.h5")
labels = label_text("/home/hussain/project2_ws/src/self_driving_car_01/scripts/label_names.csv")
offset = 5
...
def road_sign_viewer(msg: CompressedImage):
    ...
    if big is not None:
        ...
        if contourArea > 1500:
            interestedImage, sign, (x, y) = boundaryBox(img_With_Out_Line, big, 0)
            cv2.line(
                interestedImage,
                (0, lineHeight),
                (interestedImage_width, lineHeight),
            ...
            if (y < (lineHeight + offset)) or (y > (lineHeight - offset)):
                try:
                    ...
                    road_sign_node.publish(puplisher_name, labels[prediction])
                except:
                    pass
...
def main():
    road_sign_node.init_publisher(puplisher_name, "algo/sign", String)
